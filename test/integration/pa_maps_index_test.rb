...
#    @pa_map = pa_maps(:one)
  ...
  test "index including delete links" do
    ...
    assert_select 'a[href=?]', new_product_pa_map_path, remote: true, text: '新規'
    PaMap.where(product_id: @product_id).each do |pa_map|
      assert_select 'a[href=?]', product_pa_map_path(pa_map.auction_id), text: '削除', remote: true, method: :delete
    end
