...
class PcMapsIndexTest < ActionDispatch::IntegrationTest
  ...
  test "index including delete links" do
    ...
    assert_select 'a[href=?]', new_product_pc_map_path, remote: true, text: '新規'
    PcMap.where(product_id: @product_id).each do |pc_map|
      assert_select 'a[href=?]', product_pc_map_path(pc_map.custom_id), text: '削除', remote: true, method: :delete
    end
